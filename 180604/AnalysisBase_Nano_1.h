...
//#if VER == 0
//    maxSubjetCSV .assign(data.fatjet.size, 0);
//#endif
    ...
    while(data.fatjet.Loop()) {
      ...
      // N-subjettiness
#if VER == 0
      if (data.fatjet.tau1[i]>0) tau21[i] = data.fatjet.tau2[i]/data.fatjet.tau1[i];
      if (data.fatjet.tau1[i]>0) tau31[i] = data.fatjet.tau3[i]/data.fatjet.tau1[i];
      if (data.fatjet.tau2[i]>0) tau32[i] = data.fatjet.tau3[i]/data.fatjet.tau2[i];
/*
      // Maximum Subjet btag discriminator
      maxSubjetCSV[i] = -9999;
      int i_sj0 = data.fatjet.subJetIdx1[i], i_sj1 = data.fatjet.subJetIdx2[i];
...
*/
    ...
    while(data.ele.Loop()) {
      ...
      TLorentzVector ele_v4; ele_v4.SetPtEtaPhiE(data.ele.pt[i], data.ele.eta[i], data.ele.phi[i], data.ele.mass[i]);
      float pt = data.ele.pt[i];
      float abseta = std::abs(data.ele.eta[i]);
      float miniIso = data.ele.miniPFRelIso_all[i];
#if USE_POG_ID == 0
      float absIso  = data.ele.pfRelIso03_all[i]*pt;
#endif
      float absd0 = std::abs(data.ele.dxy[i]);
      float absdz = std::abs(data.ele.dz[i]);
      float ipsig = std::abs(data.ele.ip3d[i])/data.ele.sip3d[i];
#if USE_MVA_ID == 1
      ...
      int categ = data.ele.vidNestedWPBitmap[i];
      double mva = categ<3 ? data.ele.vidNestedWPBitmap[i] : data.ele.mvaFall17noIso[i];
...
#endif
      bool id_loose_noiso = (data.ele.mvaFall17noIso_WPL[i] == 1.0);
      bool id_select_noiso = (data.ele.mvaFall17noIso_WP90[i] == 1.0);
      bool id_tight  = (data.ele.mvaFall17noIso_WP80[i] == 1.0);
    ...
    while(data.mu.Loop()) {
      size_t i = data.mu.it;
      TLorentzVector mu_v4; mu_v4.SetPtEtaPhiE(data.mu.pt[i], data.mu.eta[i], data.mu.phi[i], data.mu.mass[i]);
      float pt = data.mu.pt[i];
      float abseta = std::abs(data.mu.eta[i]);
      float miniIso = data.mu.miniPFRelIso_all[i];
      float relIso = data.mu.pfRelIso04_all[i];
#if USE_POG_ID == 0
      float absIso  = data.mu.pfRelIso04_all[i]*pt;
#endif
      float absd0 = std::abs(data.mu.dxy[i]);
      float absdz = std::abs(data.mu.dz[i]);
      float ipsig = std::abs(data.mu.ip3d[i])/data.mu.sip3d[i];
      bool id_veto_noiso   = (data.mu.softId[i] == 1.0);
      bool id_loose_noiso  = (data.mu.softId[i] == 1.0);
      bool id_select_noiso = (data.mu.mediumId[i] == 1.0);
      bool id_tight_noiso  = (data.mu.tightId[i] == 1.0);
      // Veto
#if USE_POG_ID > 0
      if (passMuVeto[i] =
	  (id_veto_noiso &&
    ...
    while(data.tau.Loop()) {
      ...
      bool id_veto = (data.tau.rawIso[i] == 1);
      // Veto
      if (passTauVeto[i] = 
	  ( id_veto //&&
	    //	    pt      >= TAU_VETO_PT_CUT &&
   ...
   /*
	...
	lep1.SetPtEtaPhiE(data.mu.pt[iMuSelect[0]], data.mu.eta[iMuSelect[0]], data.mu.phi[iMuSelect[0]], data.mu.mass[iMuSelect[0]]);
	lep2.SetPtEtaPhiE(data.mu.pt[iMuSelect[1]], data.mu.eta[iMuSelect[1]], data.mu.phi[iMuSelect[1]], data.mu.mass[iMuSelect[1]]);
	lep_pair = lep1+lep2;
	M_ll = lep_pair.M();
	dPhi_ll_met = std::abs(TVector2::Phi_mpi_pi(lep_pair.Phi() - data.met.Phi[0]));
      }
...
*/
  ...
  while(data.pho.Loop()) {
    size_t i = data.pho.it;
    TLorentzVector pho_v4; pho_v4.SetPtEtaPhiE(data.pho.pt[i], data.pho.eta[i], data.pho.phi[i], data.pho.mass[i]);
    float pt = data.pho.pt[i];
    float abseta = std::abs(data.pho.eta[i]);
    bool ele_veto = (data.pho.electronVeto[i]==1);
    bool id_select = data.pho.mvaID[i];
#if VER < 4
...
//    ChargedHadronIsoEACorr.push_back(data.pho.ChargedHadronIsoEAcorrectedsp15[i]);
    ...
    if ( std::abs(data.pho.isScEtaEB[i])<1.479 ) {
      // Barrel cuts (EB)
      id_PreSel = 
  	data.pho.hoe[i]                          < 0.0396 &&
  	//data.pho.SigmaIEtaIEta[i]                   < 0.01022 &&
  	//ChargedHadronIsoEACorr[i]                   < 0.441 &&
  	data.pho.pfRelIso03_all[i] < 2.725+0.0148*pt+0.000017*pt*pt &&
  	data.pho.pfRelIso03_chg[i]        < 2.571+0.0047*pt;
    } else {
      // Encap cuts (EE)
      id_PreSel = 
  	data.pho.hoe[i]                          < 0.0219 &&
  	//data.pho.SigmaIEtaIEta[i]                   < 0.03001 &&
  	//ChargedHadronIsoEACorr[i]                   < 0.442 &&
  	data.pho.pfRelIso03_all[i] < 1.715+0.0163*pt+0.000014*pt*pt &&
  	data.pho.pfRelIso03_chg[i]        < 3.863+0.0034*pt;
    }
    ...
    if ( passPhotonPreSelect[i] = 
	 ( id_PreSel &&
      ...
      if ( passPhotonFake[i] = 
	   (data.pho.isScEtaEE[i] >= (std::abs(data.pho.isScEtaEB[i])<1.479 ? 0.01022 : 0.03001) ) ) {
  ...
  while(data.jet.Loop()) {
    size_t i = data.jet.it;
    TLorentzVector jet_v4; jet_v4.SetPtEtaPhiE(data.jet.pt[i], data.jet.eta[i], data.jet.phi[i], data.jet.mass[i]);
    // Jet ID
    if ( passLooseJet[i] = 
	 ( data.jet.jetId[i] == 1 &&
	   data.jet.pt[i]         >= JET_AK4_PT_CUT &&
	   std::abs(data.jet.eta[i])  <  JET_AK4_ETA_CUT ) ) {
	...
	if (dR<AK4_muonDR[i]) {
      ...
      nJet++;
      AK4_Ht += data.jet.pt[data.jet.it];

      // B tagging
      if (passLooseBTag[i]  = (data.jet.btagCSVV2[i] >= B_CSV_LOOSE_CUT ) ) {
	iLooseBTag.push_back(i);
	itLooseBTag[i] = nLooseBTag++;
      }
      if (passMediumBTag[i] = (data.jet.btagCSVV2[i] >= B_CSV_MEDIUM_CUT) ) {
	iMediumBTag.push_back(i);
	itMediumBTag[i] = nMediumBTag++;
      }
      if (passTightBTag[i]  = (data.jet.btagCSVV2[i] >= B_CSV_TIGHT_CUT ) ) {
	...
	itTightBTag[i] = nTightBTag++;
      ...
      // minDeltaPhi
      if (nJet<=4) {
	double dphi = std::abs(TVector2::Phi_mpi_pi(data.met.phi[0] - data.jet.phi[i]));
...
*/
	...
	double pt_ratio = selected_photons[0].Pt()/jet_v4.Pt(); // BURAYI BEN EKLEDIM
	//if (passLooseJetNoPho[i] = (dR>=0.4)) {
	//if (passLooseJetNoPho[i] = (dR>=0.4 && pt_ratio>=0.5&&pt_ratio<2.0)) {
	if (passLooseJetNoPho[i] = !(dR<0.4 && pt_ratio>=0.5&&pt_ratio<2.0)) {
	  nJetNoPho++;
	  if (data.jet.btagCSVV2[i] >= B_CSV_MEDIUM_CUT) nMediumBTagNoPho++;
	...
	data.jet.pt[i]         >  30 &&
	std::abs(data.jet.eta[i])  <  3.0 ) {
      AK4_HtOnline += data.jet.pt[data.jet.it];
  ...
  while(data.ele.Loop()) {
    ...
    // Nearest jet info
    while(data.jet.Loop()) {
      size_t j = data.jet.it;
      if (passLooseJet[j]) {
	TLorentzVector jet_v4; jet_v4.SetPtEtaPhiE(data.jet.pt[j], data.jet.eta[j], data.jet.phi[j], data.jet.mass[j]);
	double dR = ele_v4.DeltaR(jet_v4);
	if (dR<eleJetDR[i]) {
	  eleJetDR[i] = dR;
	  eleJetPt[i] = data.jet.pt[j];
	}
  ...
  while(data.fatjet.Loop()) {
    size_t i = data.fatjet.it;
    TLorentzVector AK8_v4; AK8_v4.SetPtEtaPhiE(data.fatjet.pt[i], data.fatjet.eta[i], data.fatjet.phi[i], data.fatjet.mass[i]);
...
/*
#if VER == 0
    double sd_mass_w = isData ? data.fatjet.msoftdrop[i] : softDropMassCorr[i];
    double sd_mass_top = data.fatjet.msoftdrop[i];

#elif VER == 1
    double sd_mass_w = isData ? data.fatjet.softDropMassPuppi[i] : softDropMassCorr[i];
    double sd_mass_top = data.fatjet.softDropMassPuppi[i];
#else
    double sd_mass_w = isData ? data.fatjet.uncorrSDMassPuppi[i] : softDropMassCorr[i];
    double sd_mass_top = data.fatjet.softDropMassPuppi[i];
...
*/
    ...
    if ( passLooseJetAK8[i] = 
	 ( data.fatjet.jetId[i] == 1 &&
	   data.fatjet.pt[i]         >= JET_AK8_PT_CUT &&
	...
	if (dR<AK8_muonDR[i]) {
      ...
      // Tagging Variables
      double pt      = data.fatjet.pt[i];
      double abseta  = data.fatjet.eta[i];
	...
	if (passTightWTag[i] = (tau_21 < W_TAU21_TIGHT_CUT) ) {
	  ...
	  while(data.jet.Loop()) {
	    size_t i = data.jet.it;
	    TLorentzVector AK4_v4; AK4_v4.SetPtEtaPhiE(data.jet.pt[i], data.jet.eta[i], data.jet.phi[i], data.jet.mass[i]);
	    if (passMediumBTag[i]) {
    ...
    if ( data.fatjet.pt[i]         > 150 &&
	 std::abs(data.fatjet.eta[i])  <  2.5 ) {
      // Ht
...
*/
  ...
  if (hemis_AK4.size()==2) {
    ...
    shifted_met.SetPtEtaPhi(data.met.pt[0], 0, data.met.phi[0]); // shifted_met is complicated one for us because of the lack of exact met variable on new Ntuple
  ...
  if (runOnSkim) {
    if (syst_index == 0) {
      ...
      if (TString(sample).Contains("MET")) {
        ...
        if (d.hlt.PFMET120_PFMHT120_IDTight_CaloBTagCSV_3p1==1&&nLepVeto==0&&nTauVeto==0) pass_aux_trigger = 1;   	//New ntuple (NANO) with new MET/    
        ...
        if ((d.hlt.Ele20_WPLoose_Gsf==1||d.hlt.Ele27_WPTight_Gsf==1)&&nEleVeto==1&&nMuVeto==0) pass_aux_trigger = 1;
	...
	bool OR_HLT_Photon = 
	  ...
	  d.hlt.Photon20_HoverELoose==1||
	  d.hlt.Photon30_HoverELoose==1||
   	  d.hlt.Photon40_HoverELoose==1||
  	  d.hlt.Photon50_HoverELoose==1||
 	  d.hlt.Photon60_HoverELoose==1||
          d.hlt.Photon50_R9Id90_HE10_IsoM==1 ||
	  d.hlt.Photon75_R9Id90_HE10_IsoM==1 ||
          d.hlt.Photon90_R9Id90_HE10_IsoM==1 ||
          d.hlt.Photon120_R9Id90_HE10_IsoM==1 ||
    	  d.hlt.Photon165_R9Id90_HE10_IsoM==1 ||
          d.hlt.Photon60_R9Id90_CaloIdL_IsoL==1 ||
          d.hlt.Photon60_R9Id90_CaloIdL_IsoL_DisplacedIdL==1 ||
    	  d.hlt.Photon60_R9Id90_CaloIdL_IsoL_DisplacedIdL_PFHT350MinPFJet15==1; 
	...
	MR = MR_pho;
      ...
      if (pass_aux_trigger) {
        if (nJetAK8>=1 && njet>=3 && MR>=800 && R2>=0.08) {
          if (d.hlt.AK8PFJet450==1 || d.hlt.PFHT800_PFMET75_PFMHT75_IDTight==1 || d.hlt.PFHT890==1) {
...
AnalysisBase::get_xsec_from_ntuple(const std::vector<std::string>& filenames, const bool& runOnSkim)
{
  ...
  for (const auto& filename : filenames) {
    ...
    TTree* tree = (TTree*)f->Get(runOnSkim ? "Events" : "Events");
